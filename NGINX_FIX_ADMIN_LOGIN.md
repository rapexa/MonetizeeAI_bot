# 🔧 راهنمای رفع مشکل Admin Login

## مشکل
درخواست‌های `/api/v1/admin/auth/login` به endpoint نمی‌رسند و HTML (index.html) برمی‌گردانند.

## علت
احتمالاً nginx همه درخواست‌ها را به frontend می‌دهد و به backend نمی‌رسد.

## راه حل

### 1. بررسی تنظیمات nginx

فایل nginx config خود را پیدا کنید (معمولاً `/etc/nginx/sites-available/default` یا `/etc/nginx/nginx.conf`):

```bash
sudo nano /etc/nginx/sites-available/default
```

### 2. اضافه کردن location block برای API

**مهم**: location block برای `/api/` باید **قبل از** location `/` قرار بگیرد!
